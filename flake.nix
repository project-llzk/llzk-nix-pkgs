  ...
  outputs =
    ...
    in
    {
      overlays.default = final: prev: {
        llzk-llvmPackages =
          (import ./packages/llzk_llvm/default.nix {
            llvmPackages = final.${llvmPackagesAttr};
          })
            final;

        llzk-llvmPackages-debug =
          (import ./packages/llzk_llvm/default.nix {
            llvmPackages = final.${llvmPackagesAttr};
      ...
      let
        ...
        mkClangToolsClosureCheck =
          let
            ...
            expectedLibllvm = discardContext (toString pkgs.llzk-llvmPackages.libllvm.lib);

            forbiddenPaths = map (output: discardContext (toString outerLlvm.${output})) outerLlvm.outputs;
          in
